# BackgroundErase PowerShell Integration

A minimal PowerShell script that uploads an image to the BackgroundErase API and saves the cutout (PNG with transparency) to disk.
...
- Internet access to https://api.backgrounderase.com
...
```bash
svn export https://github.com/BackgroundErase/api-integrations/trunk/PowerShell
cd PowerShell
```
...
curl -L -o background_removal.ps1 \
  https://raw.githubusercontent.com/BackgroundErase/api-integrations/main/PowerShell/background_removal.ps1
```
...
- -ApiKey: Your BackgroundErase API key
...
pwsh ./background_removal.ps1 -Src "input.jpg" -Dst "output.png" -ApiKey $env:BG_ERASE_API_KEY
```
...
Get-ChildItem -File -Include *.jpg,*.jpeg | ForEach-Object {
  ...
  pwsh ./background_removal.ps1 -Src $src -Dst $dst -ApiKey $env:BG_ERASE_API_KEY
}
```
...
pwsh ./background_removal.ps1 -Src "./input.jpg" -Dst "./output.png" -ApiKey "$BG_ERASE_API_KEY"
```
...
- Network/Proxy issues:
  - Confirm your environment can reach https://api.backgrounderase.com
- Error output:
...
- Endpoint: POST https://api.backgrounderase.com/v2
